  ...
  stages {
      ...
      steps {
        ...
        script {
          docker.withRegistry('https://index.docker.io/v1/', 'dockerlogin') {
            def workerImage = docker.build("luanfsf/worker:v${env.BUILD_ID}", './worker')
            workerImage.push()
    ...
    stage('result-docker-package') {
      ...
      steps {
        ...
        script {
          docker.withRegistry('https://index.docker.io/v1/', 'dockerlogin') {
            def resultImage = docker.build("luanfsf/result:v${env.BUILD_ID}", './result')
            resultImage.push()
      ...
      steps {
        ...
        script {
          docker.withRegistry('https://index.docker.io/v1/', 'dockerlogin') {
            ...
            def voteImage = docker.build("luanfsf/vote:${env.GIT_COMMIT}", './vote')
            voteImage.push()
